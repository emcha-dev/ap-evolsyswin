...
$entreprise = 'ValorElec'
$ID = 'VE'
$listeServices = @('Commercial','Com','Direction','Dir','Recherche et Développement','R&D')
$listeStatuts = @('Employés','Responsables')

$chemin = "ou=01_Utilisateurs,ou=$entreprise,ou=04_Entreprises clientes,ou=ETP CHasseneuil,dc=TiersLieux86,dc=fr"
$fichiersPersonnels = "\\srv-intra\FICHIERS_PERSONNELS"
$lettreDossierPerso = "Y:"
$fichierCSV = "./VE_utilisateurs.csv"
            ...
            if ($user.service -eq $listeServices[$i]){
                foreach ($statut in $listeStatuts){
                    if ($user.statut -eq $statut){    
                        $index=$i+1
                        $nomGroupe = 'G_'+$ID+'_'+$listeServices[$index]+'_'+$statut    
